...
public class ParseURL {
	...
	public static void saveDoc(Document document, String url) {
		File html_path = new File("src/webfiles/");
		
		try {
			PrintWriter html = new PrintWriter(
					new FileWriter(html_path + document.title().replace('/', '_') + ".html"));
			html.write(document.toString());
			html.close();
			htmlToText(html_path + document.title().replace('/', '_') + ".html", url,
	...
	public static void htmlToText(String htmlfile, String url, String filename) throws Exception {
		File text_path = new File("src/textfiles/");
		File file = new File(htmlfile);
		Document document = Jsoup.parse(file, "UTF-8");
		String data = document.text().toLowerCase();
		data = url + "::" + data;
		PrintWriter writer = new PrintWriter(text_path + filename);
		writer.println(data);
		writer.close();
	}
}
